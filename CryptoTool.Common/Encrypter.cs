...
namespace CryptoTool.Common
{
    ...
    public class Encrypter
    {
        ...
        /// <param name="noSignData">需要签名的字符串（未计算哈希值）</param>
        /// <param name="signAlgorithm">签名算法</param>
        /// <param name="signData">哈希值签名后的值</param>
        /// <returns></returns>
        public static bool VerifySign(string filePath, string password, string noSignData, string signAlgorithm, byte[] signData)
        {
            // 找到证书文件
            ...
            bool bVerify = oRSA4.VerifyData(messagebytes, signAlgorithm, signData);
        ...
        public static byte[] SignData(string filePath, string password, string noSignData, string signAlgorithm)
        {
            // 找到证书文件
            X509Certificate2 objx5092 = new X509Certificate2(filePath, password);
            ...
            // 对哈希进行加签
            byte[] signBytes = sign.CreateSignature(hashbytes);

            // 将加签后的数据进行base64编码，返回的就是已签名的数据 
            // string signData = Convert.ToBase64String(signBytes);
            return signBytes;
        }
    }
}